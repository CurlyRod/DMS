...
$files = $conn->query("SELECT * FROM tbl_files where folder_id = $folder_parent  order by name asc");
  ...
  <div class="modal-dialog">
    ...
    <div class="col-3"> 
        <h1 class="h3  "><span><img src="./dashboard/images/Linkfolder.png" alt="" style="height:40px;width:40px;"> </span>SHARED FOLDERS</h1>
        </div> 
        ...
        <div class="col-9"  >
             ...
             <?php 
				...
				while($id > 0){
					...
					echo '<script>
						$("#paths").prepend("<a href=\"admindashboard.php?page=adminlinkfolder&fid='.$path['id'].'\">'.$path['foldername'].'</a>/")
					</script>';
					$id = $path['parent_id'];

				}
				echo '<script>
						$("#paths").prepend("Directory: <a href=\"admindashboard.php?page=adminlinkfolder\">Home</a>/")
					</script>';
				?>
					</div>
...
location.href = 'admindashboard.php?page=adminlinkfolder&fid='+$(this).attr('data-id')
})     
$('#addfolders').click(function(){ 
	 var folder  = <?php echo isset($_GET['fid']) ? $_GET['fid'] :'' ?>  
	//  localStorage.setItem("getvalue",folder);
	location.href = 'admindashboard.php?page=adminfileupload&fid='+folder;
...
$(document).on('click','#unlinkFolder',function(){ 
    ...
    var flderid = $(this).attr("data-id");
         location.href=('admindashboard.php?page=adminupdatesharedfolder&uqfolderid='+flderid );
