...
<h1 class="h3  "><span><img src="./dashboard/images/links.png"" alt="" style="height:40px;width:40px;"> </span>RECEIVE SHARED FILES</h1>
     ...
     <?php include('db_connection.php') ;
        $files = $conn->query("SELECT a.role as type, a.firstname as uname,a.lastname as lname, f.*FROM tbl_files f inner join tbl_share u on u.user_id = f.user_id INNER join tbl_users as a ON f.user_id =a.id WHERE f.archive = 0 and u.public_to ='".$_SESSION['id']."' and f.id = u.file_id  GROUP BY f.id order by date(f.date_updated) desc");
    ...
    $(document).on('click','.deleteFile',function(e){   
         ...
         var fileId = $(this).val();
         location.href=('admindashboard.php?page=adminunsharefile&uqfileid='+fileId);
