...
<h1 class="h3  "><span><img src="./img/sidebar/totalfile.png" alt="" style="height:40px;width:40px;"> </span>ARCHIVE FILES</h1>
     ...
     <?php include('db_connection.php') ;
        $files = $conn->query("SELECT f.*,u.firstname as uname, u.lastname as lname,f.local_path  FROM tbl_files  f inner join tbl_users u on u.id = f.user_id where f.archive = 3 and f.user_id = '".$_SESSION['id']."' order by date(f.date_updated) desc;");
       ...
       if($days==0)
       {
        ...
        $query = "DELETE FROM tbl_files WHERE archive  =  3 and date_updated >'$date' and user_id=".$_SESSION['id'];  
                        ...
                        ?>
                            ...
                            <td class="text-center"><i><?php echo ucwords($row['uname'].' '.$row['lname']) ?></i></td>
                                ...
                                <td  class="text-center"><i class="to_file"><?php echo $row['file_type'] ?></i></td>
                                 ...
                                 <td> 
                                 <!-- <td  class="text-center"><i class="to_file"><?php echo $row['local_path'] ?></i></td> -->
