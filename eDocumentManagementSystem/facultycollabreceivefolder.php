...
// $folders = $conn->query("SELECT a.role as type, a.firstname as uname,a.lastname as lname, u.date_shared as dateshared,f.*FROM tbl_folders f inner join tbl_share_colab_folder u on u.user_id = f.user_id  INNER join tbl_users as a ON u.user_id = a.id WHERE u.parent_id = 0 and f.archive = 0 and u.public_to ='".$_SESSION['id']."' and f.id = u.folder_id  and f.parent_id =$imitate_parent  order by date(f.date_created) desc");

$folders = $conn->query("SELECT a.role as type, a.firstname as uname,a.lastname as lname, u.date_created as dateshared,f.*FROM tbl_colab_folder f inner join tbl_share_folder_collab as u on u.creator_id = f.creator_id INNER join tbl_users as a ON u.creator_id = a.id WHERE u.parent_id = 0 and f.archive = 0 and u.public_to ='".$_SESSION['id']."' and f.id = u.folder_id and f.parent_colab_id = $folder_parent group by f.id order by date(f.date_created) desc");
...
$files = $conn->query("SELECT a.role as type, a.firstname as uname,a.lastname as lname, f.*FROM tbl_files f INNER join tbl_users as a ON f.user_id =a.id WHERE f.folder_id = $folder_parent GROUP BY f.id order by date(f.date_updated) desc");
  ...
  <div class="modal-dialog">
    ...
    <div class="col-3"> 
        <h1 class="h3  "><span><img src="./dashboard/images/teamwork1.png"  alt="" style="height:40px;width:40px;"> </span>TEAMS</h1>
        </div> 
        ...
        <div class="col-9"  >
             ...
             <?php 
				...
				while($id > 0){
					...
					echo '<script>
						$("#paths").prepend("<a href=\"facultydashboard.php?page=facultycollabreceivefolder&fid='.$path['id'].'\">'.$path['foldername'].'</a>/")
					</script>';
					$id = $path['parent_colab_id'];

				}
				echo '<script>
						$("#paths").prepend("Directory: <a href=\"facultydashboard.php?page=facultycollabreceivefolder\">Home</a>/")
					</script>';
				?>
					</div>
					...
					<div class="row">
						<div class="col-12">
                         ...
                         <a class="addfolder ms-1"id="addFiles" ><img  src="./src/img/photos/f (1).png "style="height:40px; width:40px;"></a>
                        ...
                        <?php 
                            ...
                            ?>
                                <div class="card text-center col-md-3 p-1  folder-item  " id="cardfiles" data-id="<?php echo $row['id'] ?>" style="box-shadow:none;">
                                    <div class="card-body">
                                    <large><img src="./img/photos/yellow.png" style="height:95px; width:100x;"> <p class="to_folder fw-bold " style="font-size:13px; ">[<?php echo $row['type']?>~ <?php echo $row['uname'].' '.$row['lname'] ?>]
                        ...
                        <td><large><span><i class="fa <?php echo $icon ?>"></i></span><b class="to_file"> <?php echo $name ?></b></large>
							...
							<td>
								...
								<li><a class="dropdown-item "><button class="viewFile btn" value='<?php echo $row['id']?>'><img src="./dashboard/images/eye.png" style="width:20px;heigth:20px;"> 
									...
									Edit</button></a></li>  
                                    <li><a class="dropdown-item"><button class="shareFile btn" value='<?php echo $row['id']?>'><img src="./dashboard/images/sharing.png" style="width:20px;heigth:20px;"> 
									Share to</button></a></li> 
...
<a href="javascript:void(0)" class="custom-menu-list file-option delete "><span><i class="fa-solid fa-share-nodes"></i></span>Unlink</a> 
</div>
...
location.href = 'facultydashboard.php?page=facultycollabreceivefolder&fid='+$(this).attr('data-id')
})     
$('#addfolders').click(function(){ 
	 var folder  = <?php echo isset($_GET['fid']) ? $_GET['fid'] :'' ?>  
	//  localStorage.setItem("getvalue",folder);
	location.href = 'facultydashboard.php?page=fileupload&fid='+folder;
...
let folder_id = document.getElementById("flder_id").value;
...
console.log(value);
location.href = 'facultydashboard.php?page=fileupload&fid='+ value;
}) 
//FILE
