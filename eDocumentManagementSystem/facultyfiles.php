...
<?php 
    ...
    $folders = $conn->query("SELECT * FROM tbl_folders where parent_id = $folder_parent and archive = 0  and user_id = '".$_SESSION['id']."'  order by foldername asc");
  ...
  { 
    ...
    $files = $conn->query("SELECT * FROM tbl_files where archive = 0  and folder_id = $folder_parent  and user_id = '".$_SESSION['id']."'  order by name asc");
        ...
        <div class="col-9"  >
             ...
             <?php 
				...
				while($id > 0){
					...
					echo '<script>
						$("#paths").prepend("<a href=\"facultydashboard.php?page=facultyfiles&fid='.$path['id'].'\">'.$path['foldername'].'</a>/")
					</script>';
					$id = $path['parent_id'];

				}
				echo '<script>
						$("#paths").prepend("Directory: <a href=\"facultydashboard.php?page=facultyfiles\">Home</a>/")
					</script>';
				?>
					</div>
...
location.href = 'facultydashboard.php?page=facultyfiles&fid='+$(this).attr('data-id')
})      
...
console.log(value);
 location.href = 'facultydashboard.php?page=facultyfileupload&fid='+ value;
})   
...
// alert(flder_id); 
// var fileId = $(this).val();
location.href=('facultydashboard.php?page=facultysharefolder&folderid='+flder_id);
	...
	$(document).on('click','.shareFile',function(){ 
		...
		var fileId = $(this).val();
		location.href=('facultydashboard.php?page=facultyfileshare&fileid='+fileId);
