...
<?php 
 ...
 $folders = $conn->query("SELECT a.role as type, a.username as uname,a.lastname as lname,u.user_id,u.date_shared as dateshare ,f.*FROM tbl_folders f inner join tbl_share_folder u on u.user_id = f.user_id INNER join tbl_users as a ON a.id = u.public_to WHERE f.parent_id >= $folder_parent and f.archive = 0  and u.user_id = '".$_SESSION['id']."' and f.id = u.folder_id  order  by date(u.date_shared) desc");
//  $folders = $conn->query("SELECT a.role as type, a.firstname as uname,a.lastname as lname, u.date_shared as dateshared,f.*FROM tbl_folders f inner join tbl_share_folder u on u.user_id = f.user_id INNER join tbl_users as a ON f.user_id =a.id WHERE f.parent_id >= $folder_parent  and f.archive = 0 and u.public_to ='".$_SESSION['id']."' and f.id = u.folder_id  GROUP BY f.id order by date(f.date_created) desc");

//  $folders = $conn->query("SELECT a.role as type, a.firstname as uname,a.lastname as lname, u.date_shared as dateshared,f.*FROM tbl_folders f inner join tbl_share_folder u on u.user_id = f.user_id INNER join tbl_users as a ON f.user_id =a.id WHERE f.parent_id >= $folder_parent  and f.archive = 0 and u.public_to ='".$_SESSION['id']."' and f.id = u.folder_id  GROUP BY f.id order by date(f.date_created) desc");

$files = $conn->query("SELECT * FROM tbl_files where folder_id = $folder_parent  order by name asc");
  ...
  <div class="modal-dialog">
    ...
    <div class="col-3"> 
        <h1 class="h3  "><span><img src="./img/sidebar/paper.png" alt="" style="height:40px;width:40px;"> </span>FILES</h1>
        </div> 
        ...
        <div class="col-9"  >
             ...
             <?php 
				...
				while($id > 0){
					...
					echo '<script>
						$("#paths").prepend("<a href=\"facultydashboard.php?page=userslinkfolder&fid='.$path['id'].'\">'.$path['foldername'].'</a>/")
					</script>';
					$id = $path['parent_id'];

				}
				echo '<script>
						$("#paths").prepend("Directory: <a href=\"facultydashboard.php?page=userslinkfolder\">Home</a>/")
					</script>';
				?>
					</div>
        ...
        <div class="input-group-append">
              <span class="input-group-text bg-transparent border-0"  id="inputGroup-sizing-sm"><img src="./dashboard/images/search.png "style="height:25px;width:30px;" alt=""></span>
                        ...
                        <?php 
                            ...
                            ?>
                                <div class="card text-center col-md-2 p-1  folder-item  " id="cardfiles" data-id="<?php echo $row['id'] ?>" style="box-shadow:none;">
                                    <div class="card-body">
                                            <large><img src="./img/photos/yellow.png" style="height:95px; width:100x;"> <p class="to_folder  " style="font-size:14px; ">[<?php echo $row['uname'].' '.$row['lname'] ?>]
...
<a href="" class="custom-menu-list file-option delete" id="unlinkFolder"><span><i class="fa-solid fa-link mr-2 text-danger"></i></span>Update Share</a> 
</div>
...
location.href = 'facultydashboard.php?page=userslinkfolder&fid='+$(this).attr('data-id')
})     
...
$(document).on('click','#unlinkFolder',function(){ 
    ...
    var flderid = $(this).attr("data-id");
         location.href=('facultydashboard.php?page=facultyupdate&uqfolderid='+flderid );
