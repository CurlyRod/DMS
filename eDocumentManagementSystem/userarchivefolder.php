...
$folders = $conn->query("SELECT * FROM tbl_folders where parent_id = $folder_parent and archive= 1 and user_id = '".$_SESSION['id']."'  order by foldername asc");
...
<div class="modal fade" id="updateFolderModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <div class="col-3"> 
        <h1 class="h3  "><span><img src="./dashboard/images/deleted.png" alt="" style="height:40px;width:40px;"> </span>ARCHIVE FOLDERS</h1>
        </div> 
        ...
        <div class="col-9"  >
             ...
             <?php 
				...
				while($id > 0){
					...
					echo '<script>
						$("#paths").prepend("<a href=\"userdashboard.php?page=userarchivefolder&fid='.$path['id'].'\">'.$path['foldername'].'</a>/")
					</script>';
					$id = $path['parent_id'];

				}
				echo '<script>
						$("#paths").prepend("Directory: <a href=\"userdashboard.php?page=userarchivefolder\">Home</a>/")
					</script>';
				?>
					</div>
                 ...
                 <a class="addfolder ms-1 d-none" data-bs-toggle="modal" data-bs-target="#addFilesModal" ><img  src="./src/img/photos/f (1).png "style="height:40px; width:40px;"></a>
                 
				 <a class="addfolder ms-3 d-none" id="addfolders" data-bs-toggle="modal" data-bs-target="#addFolderModal"> 
                        ...
                        <?php 
                            ...
                            ?>
                                <div class="card text-center col-md-2 p-1  folder-item  " id="cardfiles" data-id="<?php echo $row['id'] ?>" style="box-shadow:none;">
                                    <div class="card-body">
                                            <large><img src="./img/photos/yellow.png" style="height:95px; width:100x;"> <p class="to_folder  " style="font-size:13px; "><b><?php echo $row['foldername'] ?></b></p></large>
...
<a href="javascript:void(0)" class="custom-menu-list file-option edit"id="restore"><span><i class="fa fa-repeat fa-lg" style="color:#FFAA13;margin-right:5px;"></i> Restore</span> </a>
...
location.href = 'userdashboard.php?page=userarchivefolder&fid='+$(this).attr('data-id')
})     
...
$(document).on('click','.deleteFile',function(e){   
	...
	$(document).on('click','.shareFile',function(){ 
		...
		var fileId = $(this).val();
		location.href=('userdashboard.php?page=fileshare&fileid='+fileId);
...
{ 
	...
	//alert(flder_id);
		$.ajax({  
			...
			type:"GET",
			url:"adminsavefolder.php?folderid="+flder_id,
			success: function(response){ 
			  var res = jQuery.parseJSON(response);
			  if(res.status == 404){ 
				$('#foldererrorMessage').removeClass('d-none');   
...
$(document).on('submit','#updatefolder',function(e){  
          ...
          $(document).on('click','#restore',function(e){   
              ...
              Swal.fire({
                  title: 'Are you sure to restore  this folder and its and content?',
                  icon: 'warning', 
                  width:'500px' ,  
                  showCancelButton: true,
                  cancelButtonColor: '#d33',
